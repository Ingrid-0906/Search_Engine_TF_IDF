...
class Query:
    ...
    def process_index(self):
        ...
        for x, y in relevant_docs.items():
            posicao = y['resumo'][0].lower().find(keyword)
            nova_frase = f"...{' '.join(self.keywords)}{y['resumo'][0][posicao+len(keyword):]}" if posicao != -1 else ""
            relevant_docs[x]['resumo'] = nova_frase
    ...
    def display_results(self):
        ...
        for doc in sorted(self.result_data, key=lambda k: k['score'], reverse=False)[:self.n]:
            print(json.dumps(doc, ensure_ascii=False).encode('utf-8').decode('utf-8'))
